...
# ==============================================================================
#  Nuovo filtro: sostituzione del verde con rosa "Barbie"
#  Questa funzione cerca le aree verdi nell'immagine (in base alla tonalità
#  HSV, alla saturazione e alla luminosità) e ne modifica la tonalità per
#  ottenere un rosa vivace stile Barbie. Per rendere l'effetto più gradevole
#  e vintage viene applicata anche una leggera curva a S sul canale della
#  luminosità (V). È possibile regolare i parametri di rilevazione e
#  saturazione tramite gli argomenti della funzione.
#
#  Parametri principali:
#    green_lower_deg / green_upper_deg – limiti inferiori e superiori in
#        gradi (0–360) per la tonalità HSV considerata “verde”.
#    sat_min / val_min – soglie minime di saturazione e luminosità per
#        evitare di modificare aree poco sature o troppo scure (come ombre e
#        highlights).
#    target_deg – tonalità di destinazione in gradi (0–360). 327° corrisponde
#        al rosa Barbie (#DA1884).
#    sat_boost – fattore di incremento della saturazione per le aree
#        convertite.
#    contrast – se True applica la curva a S sul canale V per un look più
#        "vintage".
#
def apply_green_to_barbie(
    bgr_img: np.ndarray,
    green_lower_deg: float = 50.0,
    green_upper_deg: float = 130.0,
    target_deg: float = 327.0,
    sat_boost: float = 1.4,
    sat_min: int = 60,
    val_min: int = 40,
    contrast: bool = True,
) -> np.ndarray:
    """Sostituisce le tonalità verdi con un rosa intenso stile Barbie.

    L'immagine viene convertita in spazio colore HSV. I pixel con tonalità
    compresa tra ``green_lower_deg`` e ``green_upper_deg`` (e con
    saturazione/valore sopra le soglie ``sat_min`` e ``val_min``) sono
    identificati come "verdi". Per questi pixel la tonalità (H) viene
    impostata a ``target_deg`` e la saturazione viene incrementata di
    ``sat_boost`` volte. A tutta l'immagine può poi essere applicato un
    leggero incremento di contrasto sul canale V tramite una curva a S.

    Args:
        bgr_img: immagine d'ingresso in formato BGR.
        green_lower_deg: limite inferiore della tonalità verde in gradi.
        green_upper_deg: limite superiore della tonalità verde in gradi.
        target_deg: tonalità di destinazione (rosa) in gradi.
        sat_boost: fattore moltiplicativo per aumentare la saturazione dei
            pixel sostituiti.
        sat_min: saturazione minima (0–255) per considerare un pixel "verde".
        val_min: valore (luminosità) minimo (0–255) per considerare un pixel "verde".
        contrast: se True applica la curva a S sul canale V.

    Returns:
        Immagine BGR con le aree verdi sostituite da rosa.
    """
    # Converti in HSV
    hsv = cv2.cvtColor(bgr_img, cv2.COLOR_BGR2HSV)
    h, s, v = cv2.split(hsv)

    # Converti la tonalità da intervallo [0,179] a gradi [0,360]
    h_deg = h.astype(np.float32) * 2.0

    # Maschera per i pixel verdi: tonalità entro l'intervallo e saturazione/valore sopra le soglie
    mask = (
        (h_deg >= green_lower_deg) & (h_deg <= green_upper_deg) &
        (s >= sat_min) & (v >= val_min)
    )

    # Nuova tonalità destinazione (scala 0–179 per OpenCV)
    target_hue = np.uint8((target_deg / 2.0) % 180)

    # Applica la nuova tonalità ai pixel verdi
    h_new = h.copy()
    h_new[mask] = target_hue

    # Incrementa la saturazione per i pixel modificati
    s_new = s.astype(np.float32)
    s_new[mask] = np.clip(s_new[mask] * sat_boost, 0, 255)

    # Ricostruisci HSV con i nuovi canali
    hsv_mod = cv2.merge([h_new, s_new.astype(np.uint8), v.copy()])

    # Applica un leggero contrasto sul canale V tramite una curva a S per un look vintage
    if contrast:
        v_channel = hsv_mod[:, :, 2]
        v_f = v_channel.astype(np.float32)
        # Normalizza 0–1
        t = v_f / 255.0
        # Funzione S-curve (3t^2 - 2t^3)
        smooth = t * t * (3.0 - 2.0 * t)
        # Costruzione della curva: valori presi dal filtro originale
        shadow_mul = 0.93
        hi_mul = 1.06
        mid_boost = 1.05
        boost = shadow_mul + (hi_mul - shadow_mul) * smooth
        v_new = v_f * boost * mid_boost
        hsv_mod[:, :, 2] = np.clip(v_new, 0, 255).astype(np.uint8)
...
try:
    ...
    # 2. Applica il nuovo filtro "green to Barbie"
    print("Elaborazione dell'immagine in corso... (conversione dei verdi in rosa Barbie)")
    filtered_img = apply_green_to_barbie(img)
    print("Filtro applicato con successo!")

    # 3. Prepara il file per il download
    # Costruisci il nuovo nome del file
    name, ext = filename.rsplit('.', 1)
    # Cambia il nome del file per riflettere la nuova elaborazione
    output_filename = f"{name}_barbie.{ext}"
